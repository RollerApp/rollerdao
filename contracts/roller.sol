...
contract RollerDAOV1 is Initializable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable {
    using SafeMath for uint256;
    ...
    function initialize(address marketingAddress_, address liquidityPoolAddress_, uint256 marketingFee_, uint256 liquidityFee_) initializer public {
        __ERC20_init("ROLLER", "ROLL");
        __ERC20Permit_init("ROLLER");
        _owner = msg.sender;
        _mint(msg.sender, 100000000 ether);
        _marketingAddress = marketingAddress_;
        _liquidityPoolAddress = liquidityPoolAddress_;
    ...
    function _transfer(
        ...
        if(_isExcluded[from] || _isExcluded[to] || (from != _liquidityPoolAddress && to != _liquidityPoolAddress)){
            ...
            uint256 _mfee = _calculateMarketingFee(amount);
            uint256 _lfee = _calculateLiquidityFee(amount);
            unchecked {
                _balances[from] = fromBalance - amount;
                _balances[to] += amount.sub(_mfee).sub(_lfee);
                _balances[_marketingAddress] = _mfee;
                _balances[_liquidityPoolAddress] = _lfee;
            }
